    ...
    try {
      ...
      filteredUsers.forEach((u) => {
        const userAppts = appointments.filter(
          (a) => a.studentId?._id === u._id,
        );
        const sortedAppts = [...userAppts].sort(
          (a, b) => new Date(b.date) - new Date(a.date),
        );
        const lastSession =
          sortedAppts.length > 0
            ? new Date(sortedAppts[0].date).toLocaleDateString("en-US", {
                month: "short",
                day: "2-digit",
                year: "numeric",
              })
      ...
      <div style={s.sidebar}>
        ...
        <div style={s.nav}>
          ...
          <NavItem
            ...
            icon={<Users size={20} />}
            label="Counselors"
            active={isActiveNav("/admin-counselors")}
          />
          <NavItem
            to="/admin-specialties"
            icon={<Award size={20} />}
            label="Specialties"
            active={isActiveNav("/admin-specialties")}
          />
          <NavItem
            to="/admin-users"
            icon={<User size={20} />}
          ...
          <NavItem
            ...
            label="Appointments"
            active={isActiveNav("/admin-appointments")}
          />
        </div>
      </div>
        ...
        <div style={s.contentScroll}>
          ...
          <div style={s.pageHeader}>
            <div>
              ...
              <p style={s.pageSub}>
                Central hub for governing practitioner and student access across
          ...
          <div style={s.actionRow}>
            <div style={s.actionLeft}>
              ...
              <button style={s.btnSecondary} onClick={handleExportPDF}>
            ...
            <table style={s.table}>
              ...
              <tbody>
                {displayUsers.map((u, i) => {
                  ...
                  const lastSession =
                    sortedAppts.length > 0
                      ? new Date(sortedAppts[0].date).toLocaleDateString(
                          "en-US",
                          { month: "short", day: "2-digit", year: "numeric" },
                        )
                      : "No sessions";
                      ...
                      <td style={{ ...s.td, textAlign: "right" }}>
                        <div style={s.actionBtns}>
                          ...
                          <button
                            ...
                            style={u.isActive ? s.btnDeactivate : s.btnActivate}
            ...
            <div style={s.modalBodyDetail}>
              <div style={s.detailAvatar}>
                ...
                <div style={{ marginTop: "0.5rem" }}>
                  <span
                    style={
                      selectedUser.isActive ? s.availActive : s.availInactive
                    }
                  >
              ...
              <div style={s.historySection}>
                <h5 style={s.sectionTitle}>
                  <Activity size={16} /> Counseling History
                </h5>
                {(() => {
                  const userAppts = appointments.filter(
                    (a) => a.studentId?._id === selectedUser._id,
                  );
                  if (userAppts.length === 0) {
                    return (
                      <div style={s.emptyHistory}>
                        No counseling history found for this student.
                      </div>
                    );
                  }
                      ...
                      {userAppts.map((appt, idx) => (
                        <div key={idx} style={s.historyCard}>
                          ...
                          <div style={s.assignedCounselor}>
                            ...
                            {appt.counselorId?.userId?.name || "Unknown"}
                          </div>
                        </div>
                      ))}
                    </div>
...
const s = {
  ...
  bannerWidget: {
    borderRadius: "12px",
    background: "linear-gradient(135deg, #d1d2d4ff 0%, #424346ff 100%)",
    position: "relative",
    overflow: "hidden",
    minHeight: "160px",
  },
  bannerOverlay: {
    ...
    inset: 0,
    backgroundImage:
      'url("https://i.pinimg.com/1200x/09/52/53/0952532c5960f2bf311d8110c7b9ce5e.jpg")',
