...
const FeedbackModal = ({ appointmentId, initialData, onClose, onFeedbackSubmitted }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (initialData?._id) {
                await api.put(`/feedback/${initialData._id}`, { rating, comment });
            } else {
                await api.post('/feedback', { appointmentId, rating, comment });
            }
            setIsSuccess(true);
            setTimeout(() => {
                onFeedbackSubmitted();
            ...
            <div style={s.modal}>
                {isSuccess ? (
                    ...
                    <>
                        ...
                        <div style={s.modalHeader}>
                            ...
                            <h3 style={s.modalTitle}>{initialData ? 'Edit Your Feedback' : 'Rate Your Session'}</h3>
                            <p style={s.modalSub}>{initialData ? 'Update your experience with this counselor.' : 'Your feedback helps counselors improve.'}</p>
                        </div>
                ...
                <form onSubmit={handleSubmit}>
                        ...
                        <button type="submit" style={{ ...s.submitBtn, opacity: submitting ? 0.75 : 1 }} disabled={submitting}>
                            {submitting ? 'Updating…' : (initialData ? 'Update Feedback' : 'Submit Feedback')}
                        </button>
                    </form>
                    </>
                )}
