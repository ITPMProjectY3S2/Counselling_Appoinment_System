...
const AdminAppointments = () => {
    ...
    const handleBulkAction = async (newStatus) => {
        ...
        try {
            await Promise.all(selectedItems.map(id => api.put(`/appointments/${id}/status`, { 
                status: newStatus,
    ...
    const submitApproval = async () => {
        try {
            await api.put(`/appointments/${appointmentToApprove._id}/status`, { 
                status: 'approved',
                meetingType,
                meetingLink
            });
...
const s = {
    // Layout specifics
    layout: { display: 'flex', flexDirection: 'column', height: '100vh', width: '100vw', backgroundColor: '#f8fafc', overflow: 'hidden', fontFamily: '"Inter", "Segoe UI", sans-serif' },
