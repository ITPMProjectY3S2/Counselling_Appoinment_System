...
const AdminCounselors = () => {
    ...
    const handleUpdateCounselor = async (e) => {
        ...
        try {
            ...
            await api.put(`/admin/counselors/${selectedCounselor._id}`, updateData);
            setViewMode(null);
                        ...
                        {viewMode === 'view' ? (
                            <div style={s.modalBodyDetail}>
                                ...
                                <div style={s.detailSection}>
                                    ...
                                    <div style={{...s.detailRow, flexDirection: 'column', alignItems: 'flex-start', marginTop: '1rem' }}>
                                        <div style={{ fontWeight: 700, marginBottom: '0.5rem' }}>Weekly Availability:</div>
                                        {selectedCounselor.weeklyAvailability && selectedCounselor.weeklyAvailability.length > 0 ? (
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', width: '100%' }}>
                                                {selectedCounselor.weeklyAvailability.map((wa, idx) => (
                                                    <div key={idx} style={{ fontSize: '0.85rem', padding: '0.5rem', background: '#f1f5f9', borderRadius: '6px' }}>
                                                        <span style={{ fontWeight: 700 }}>{wa.day}:</span> {wa.slots.join(', ')}
                                                    </div>
                                                ))}
                                            </div>
                                        ) : (
                                            <div style={{ fontSize: '0.85rem', color: '#64748b' }}>No availability set.</div>
                                        )}
                                    </div>
                                </div>
                            </div>
...
const s = {
    // Layout specifics
    layout: { display: 'flex', flexDirection: 'column', height: '100vh', width: '100%', backgroundColor: '#f8fafc', overflow: 'hidden' },
